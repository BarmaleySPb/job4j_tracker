...
public class AnalyzeTest {
    ...
    public void whenListOfSubjectAverage() {
        List<Tuple> average = Analyze.averageScoreBySubject(
                List.of(
                        new Pupil("Ivanov", List.of(new Subject("Math", 100), new Subject("Lang", 100))),
                        new Pupil("Petrov", List.of(new Subject("Math", 40), new Subject("Lang", 40))),
                        new Pupil("Sidorov", List.of(new Subject("Chem", 80), new Subject("Lang", 100))),
                        new Pupil("Petrov", List.of(new Subject("Math", 100), new Subject("Chem", 100)))
                ).stream()
        );
        assertThat(average, is(List.of(
                new Tuple("Chem", 90D),
